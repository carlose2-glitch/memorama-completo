...
let contador =0;//se encarga de sumar en 1 para poder visualizar el final de la partida 
let src;/* creado para guardar el src de las tarjetas */
let selecciones=[];/*arreglo creado para realizar el conteo de las cartas*/
let tarjetas=[];/*arreglo creado para guardar el orden en el cual va a ser impreso en pantalla las tarjetas */
...
/*imagenes que solamente sirven para hacer comparacion */
let baltazart = document.querySelector('.baltazart');
let gaspart = document.querySelector('.gaspart');
let joset = document.querySelector('.joset');
let mariat = document.querySelector('.mariat');
let melchort = document.querySelector('.melchort');
let mulat = document.querySelector('.mulat');
let ninot = document.querySelector('.ninot');
let pastort = document.querySelector('.pastort');
console.log(baltazart.src);
let img_cubiertas = [baltazart.src, gaspart.src, joset.src, mariat.src, melchort.src, mulat.src, ninot.src, pastort.src];
...
const med = document.querySelector('.med');//sirve para hacer modificaciones sobre el boton de la tarjeta navideña
const btnfinalizar = document.querySelector('.finalizar');//sirve para hacer modificaciones sobre el boton de la tarjeta navideña
...
let cubiertas = [baltazarh,gasparh,joseh,mariah,melchorh,mulah,jesush,pastorh];
...
function deseleccionar(selecciones){
    ...
    contador++;
      setTimeout(()=>{//espera un segundo para entrar 
        ...
        for(let i=0;i<=7;i++){//creado para poder hacer una busqueda mas efectiva al momento de ingresar el texto en el modal 2
          ...
          if(trasera1.src == img_cubiertas[i]){//verifica cual es el texto que le pertenece segun el personaje
  ...
  if (contador==8){
    ...
    med.style.display = 'none';//desaparece el boton de comenzar 
    btnfinalizar.style.display = 'flex';//aparece el boton de finalizar
...
function aparecerTargets(trasera1){
  ...
  for(let i =0; i<=7; i++){
      ...
      if(trasera1.src == img_cubiertas[i]){//verifica si el src es igual 
        
        cubiertas[i].classList.add('mostrar');//agrega una clase en uno de los personajes de las tarjetas cubiertas y se va a encargar de hacer la animacion de cada tarjeta cubierta
        
       setTimeout(()=>{//espera tres segundos para poder borrar la tarjeta cubierta seleccionada
          borrar.removeChild(cubiertas[i]);
 ...
 botonp.addEventListener('click',()=>{
  ...
  btnfinalizar.addEventListener('click',()=>{
    targetsModals.style.display = 'none';
    cuerpo.style.overflow = 'auto';
    generarTablero();
  })